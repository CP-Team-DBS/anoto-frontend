...
const EmotionCard = ({ emotion }: { emotion: EmotionDetails }) => {
  return (
    <div 
      className="flex flex-col items-center justify-center w-40 h-48 text-white rounded-xl"
    >
      <Image
  ...
  useEffect(() => {
    ...
    if (storedResponse) {
      try {
        ...
        const sortedEmotions = [...apiData.emotions].sort((a, b) => b.score - a.score);
        ...
        const mappedEmotions = sortedEmotions.map(e => {
          ...
          if (details) {
            ...
            console.warn(`Unknown emotion: ${e.name}. No SVG fallback provided.`);
            return {
              name: e.name,
              score: e.score,
              svgFile: ''
    ...
    <div className="flex flex-col min-h-screen bg-[#0E103D] text-white font-sans">
      <Container>
        ...
        <div className="text-center text-xl max-w-2xl mx-auto mb-12">
          ...
          {insightData.validation && <p className="mt-4 text-xl">{insightData.validation}</p>}
        </div>
